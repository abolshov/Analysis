...
from experimental.LayerUtils import Embedding, ParticleEmbedding

def main():
    lep_tensor = tf.random.normal(shape=(10, 8))
    lep_embedding = Embedding(dim_embedding=512, dropout_rate=0.01)
    ...
    jet_embedding = Embedding(dim_embedding=512, dropout_rate=0.01)
    ...
    fatjet_embedding = Embedding(dim_embedding=512, dropout_rate=0.01)
    ...
    met_embedding = Embedding(dim_embedding=512, dropout_rate=0.01)
    ...
    particle_embedding = ParticleEmbedding(dim_embedding=512, dropout_rate=0.01)
