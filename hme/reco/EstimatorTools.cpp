...
{
    double deta = eta - l.Eta();
    double cos_dphi = std::cosh(deta) - (mw*mw)/(2.0*met.Pt()*l.Pt());

    if (std::abs(cos_dphi) > 1.0)
    {
        return std::nullopt;
    }
    ...
    TLorentzVector v;
    v.SetPtEtaPhiM(met.Pt(), eta, l.Phi() + dphi , 0.0);
    return std::make_optional<TLorentzVector>(std::move(v));
}
...
OptionalPair Experimental::EstimateMassIdealNu(std::vector<TLorentzVector> const& particles, std::unique_ptr<TH1F>& lead_b_pdf, TRandom3& rg, int evt, TLorentzVector const& nu)
{
    ...
    [[maybe_unused]] double W_dijet_mass = (j1 + j2).M();
...
OptionalPair Experimental::EstimateMassNoMETCorr(std::vector<TLorentzVector> const& particles, std::unique_ptr<TH1F>& pdf, TRandom3& rg, int evt)
{
    TLorentzVector b1 = particles[PhysObj::bj1];
    TLorentzVector b2 = particles[PhysObj::bj2];
    TLorentzVector j1 = particles[PhysObj::wj1];
...
OptionalPair Experimental::EstimateMassRandMETFrac(std::vector<TLorentzVector> const& particles, std::unique_ptr<TH1F>& pdf, std::unique_ptr<TH1F>& met_pdf, TRandom3& rg, int evt)
{
    TLorentzVector b1 = particles[PhysObj::bj1];
    TLorentzVector b2 = particles[PhysObj::bj2];
    TLorentzVector j1 = particles[PhysObj::wj1];
    ...
    // double mw = rg.Uniform(0, mh - W_dijet_mass);
    ...
    for (int i = 0; i < N_ITER; ++i)
    {
        ...
        if (b_jet_resc)
        {
            ...
            met_corr *= f;
            std::optional<TLorentzVector> nu = ComputeNu(l, j1, j2, f*met_corr, mh, eta);
            // std::optional<TLorentzVector> nu = ComputeNu(l, met_corr, mw, eta);
    ...
    if (res_mass->GetEntries())
    {
        ...
        //     res_mass->Draw();
        //     c1->SaveAs(Form("iter_plots/gen_res_mass_evt_%d.png", evt));
        // }
...
OptionalPair Experimental::EstimateMassRandWmass(std::vector<TLorentzVector> const& particles, std::unique_ptr<TH1F>& pdf, TRandom3& rg, int evt)
{
    TLorentzVector b1 = particles[PhysObj::bj1];
    TLorentzVector b2 = particles[PhysObj::bj2];
    TLorentzVector j1 = particles[PhysObj::wj1];
    ...
    double mw = rg.Uniform(0.0, mh - W_dijet_mass);
    ...
    for (int i = 0; i < N_ITER; ++i)
    {
        ...
        if (b_jet_resc)
        {
            ...
            if (nu)
            {
                ...
                double dm = std::abs(H_WW_mass - mh);
                double w = dm < mh ? 1.0 - dm/mh : 0;
                // if (std::abs(H_WW_mass - mh) > 1.0)
                // {
                //     ++failed_iter;
                //     continue;
...
OptionalPair Experimental::EstimateMassWeightRescIdealNu(std::vector<TLorentzVector> const& particles, std::unique_ptr<TH1F>& pdf, std::unique_ptr<TH1F>& pt_corr_pdf, std::unique_ptr<TH1F>& mass_pdf, TRandom3& rg, int evt, TLorentzVector const& nu, double& mass, double& integral)
{
    TLorentzVector b1 = particles[PhysObj::bj1];
    TLorentzVector b2 = particles[PhysObj::bj2];
    TLorentzVector j1 = particles[PhysObj::wj1];
    ...
    rg.SetSeed(42);
    double mh = rg.Gaus(HIGGS_MASS, HIGGS_WIDTH);
    double Hbb_uncor_pt = (b1 + b2).Pt();

    auto res_mass = std::make_unique<TH1F>("X_mass", Form("X->HH mass: event %d", evt), N_BINS, 0.0, MAX_MASS);
    auto bad_mass = std::make_unique<TH1F>("X_mass_bad", Form("X->HH mass: event %d", evt), N_BINS, 0.0, MAX_MASS);
    ...
    for (int i = 0; i < N_ITER; ++i)
    {
        ...
        if (b_jet_resc)
        {
            ...
            TLorentzVector tmp(bb1);
            tmp += bb2;

            double r = tmp.Pt()/Hbb_uncor_pt;
            w *= pt_corr_pdf->GetBinContent(pt_corr_pdf->FindBin(r));
            w *= mass_pdf->GetBinContent(mass_pdf->FindBin(tmp.M()));
    ...
    {
        int binmax = bad_mass->GetMaximumBin(); 
        mass = bad_mass->GetXaxis()->GetBinCenter(binmax);
        integral = bad_mass->Integral();
        return std::nullopt;
    }
...
OptionalPair Experimental::EstimateMassWeightResc2dPDFIdealNu(std::vector<TLorentzVector> const& particles, std::unique_ptr<TH2F>& pdf, std::unique_ptr<TH1F>& pt_corr_pdf, std::unique_ptr<TH1F>& mass_pdf, TRandom3& rg, int evt, TLorentzVector const& nu)
{
    TLorentzVector b1 = particles[PhysObj::bj1];
    TLorentzVector b2 = particles[PhysObj::bj2];
    TLorentzVector j1 = particles[PhysObj::wj1];
    ...
    rg.SetSeed(42);
    [[maybe_unused]] double mh = rg.Gaus(HIGGS_MASS, HIGGS_WIDTH);
